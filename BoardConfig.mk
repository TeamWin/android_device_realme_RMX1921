...
TARGET_KERNEL_SOURCE := kernel/realme/sdm-710
#TARGET_KERNEL_CROSS_COMPILE_PREFIX := aarch64-linux-android-4.9
TARGET_KERNEL_CONFIG := RMX1921_defconfig
TARGET_COMPILE_WITH_MSM_KERNEL := true
KERNEL_LLVM_SUPPORT := true #TRIAL
KERNEL_SD_LLVM_SUPPORT := true #TRIAL
#KERNEL_MODULES_INSTALL := dlkm
#KERNEL_MODULES_OUT := out/target/product/RMX1921/vendor/lib/modules
...
# AVB 2.0
#PRODUCT_SUPPORTS_VERITY := true #Read only variable
...
#$(call inherit-product, build/make/target/product/gsi_keys.mk) # Include GSI keys into first-stage ramdisk, so we can enable verified boot when booting a GSI. #read only variable PRODUCT_BUILD_PROP_OVERRIDES 
...
#Cache (must not be defined in A/B)
...
#TARGET_RECOVERY_UPDATER_LIBS += librecovery_updater_msm # Enable compilation of oem-extensions to recovery #Do not use in A/B acc to lineage
...
BOARD_VENDORIMAGE_PARTITION_SIZE := 1634656256 #1073741824
...
# GPS
BOARD_VENDOR_QCOM_GPS_LOC_API_HARDWARE := sdm710
TARGET_NO_RPC := true
...
#PRODUCT_COMPATIBLE_PROPERTY_OVERRIDE := true #read only
...
#Enabled only for products launched with Q. They contain a new metadata partition as well as Super partition
#BOARD_DYNAMIC_PARTITION_ENABLE := false
#PRODUCT_USE_DYNAMIC_PARTITIONS := false #read only
#PRODUCT_PACKAGES += fastbootd #userspace implementation of fastboot protocol to understand dynamic partitions
